        ...
        for row in raw_data:
            try:
                ...
                if len(keys) >= 7:
                    ...
                    theme = row[keys[2]].strip() if keys[2] in row else ""
                    subtitle = row[keys[3]].strip() if keys[3] in row else ""
                    url = row[keys[4]].strip() if keys[4] in row else ""
                    keywords = row[keys[5]].strip() if keys[5] in row else ""
                    restriction = row[keys[6]].strip() if keys[6] in row else "無限制"
