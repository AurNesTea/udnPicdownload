...
class DataUpdater:
    ...
    def transform_data(self, raw_data: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for row in raw_data:
            try:
                # 直接使用原始欄位名稱（處理編碼問題）
                main_id = row.get('主編號', '').strip()
                sub_id = row.get('編號', '').strip()
                url = row.get('URL', '').strip()
                theme = row.get('主題', '').strip()
                subtitle = row.get('次主題（圖名）', '').strip()
                keywords = row.get('關鍵字（、區隔）', '').strip()
                restriction = row.get('使用限制', '無限制').strip()
                
                # 驗證必要欄位
                if not all([main_id, sub_id, url, theme]):
                    logger.warning(f"跳過不完整的資料行: {row}")
                    continue
                ...
                logger.error(f"處理資料行時發生錯誤: {e}, 資料: {row}")
                continue
...
"""
        ...
        for theme_num in range(1, 6):
            ...
            for img in images:
                ...
                img_id = img["id"].replace('"', '\\"').replace("'", "\\'")
                img_url = img["url"].replace('"', '\\"').replace("'", "\\'")
                img_title = img["title"].replace('"', '\\"').replace("'", "\\'")
                img_subtitle = img["subtitle"].replace('"', '\\"').replace("'", "\\'")
                img_keywords = img["keywords"].replace('"', '\\"').replace("'", "\\'")
                img_restriction = img["restriction"].replace('"', '\\"').replace("'", "\\'")
    ...
    def validate_output_file(self):
        ...
        try:
            ...
            # 嘗試執行 JavaScript 語法檢查
            exec(content.replace('const imageData = ', 'imageData = '))
